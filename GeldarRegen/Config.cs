...
namespace GeldarRegen
{
    public class Config
    {
        ...
        {
            string filepath = Path.Combine(TShock.SavePath, "worldregen.json");
            try
            {
        ...
        public static bool ReadConfig()
        {
            ...
            try
            {
                ...
                {
                    TShock.Log.ConsoleError("World Regen config not found, how about a new one?");
                    CreateConfig();
                    return true;
                }
            }
        ...
        {
            public int RegenInterval = 86400;
        }
        #endregion
        ...
        private void Reloadcfg(CommandArgs args)
        {
            if (ReadConfig())
            {
                args.Player.SendMessage("World Regen config reloaded", Color.Goldenrod);
            }
            else
            {
